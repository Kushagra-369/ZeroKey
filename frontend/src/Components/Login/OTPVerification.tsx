...
export default function OTPVerification() {
  ...
  const { email, isNewUser, deviceInfo } = location.state || {};
  ...
  const handleVerify = async (otpCode?: string) => {
    ...
    try {
      ...
      const { token, user, recoveryCodes } = res.data.data;

      // Store token & user in localStorage (for API calls)
      ...
      // ✅ ALWAYS GO TO FACE LOGIN – NO DEVICE APPROVAL
      navigate('/login/face', {
        state: {
          email,
          isNewUser,
